...
def quicksort(arr, left, right):
    if left < right:
        ...
        quicksort(arr, left, partition_pos - 1)                 #calling quicksort on all elements that are less then the pivot element
...
def partition(arr, left, right):                                #returns the index of the pivot element after the first step of quicksort
    i = left
    j = right - 1
    pivot = arr[right]
